...
class IOULoss(nn.Module):
    def forward(self, pred, target, weight=None):
        ...
        pred_center_x = pred[:, 0] + abs((pred[:, 2] - pred[:, 0])) / 2
        pred_center_y = pred[:, 1] - abs((pred[:, 1] - pred[:, 3])) / 2
        ...
        target_center_x = target[:, 0] + abs((target[:, 2] - target[:, 0])) / 2
        target_center_y = target[:, 1] - abs((target[:, 1] - target[:, 3])) / 2
        ...
        w_out = torch.max(pred_left, target_left) + \
                      torch.max(pred_right, target_right)
        h_out = torch.max(pred_bottom, target_bottom) + \
        ...
        center_dis = (pred_center_x - target_center_x)*(pred_center_x - target_center_x) + (pred_center_y - target_center_y)*(pred_center_y - target_center_y)
        # center_dis = np.square(pred_center_x - target_center_x) + np.square(pred_center_y - target_center_y)
        # outer_diagonal_line = np.square(w_out) + np.square(h_out)
        outer_diagonal_line = (w_out)*(w_out) + (h_out)*(h_out)
        dious = ious - center_dis / outer_diagonal_line
        losses = 1-dious
        # dious1 = (dious+1)/2
        # losses = -torch.log(dious1)

            #losses = -torch.log((area_intersect + 1.0) / (area_union + 1.0)+ center_dis / outer_diagonal_line)
        #losses = -torch.log((1 + dious + 0.1) / (2 + 0.1))
        #losses = -torch.log((1 - dious))
