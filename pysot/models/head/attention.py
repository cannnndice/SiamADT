...
class PAM_Module(nn.Module):
    ...
    def __init__(self, in_dim):
        ...
        self.softmax = nn.Softmax(dim=-1)#当dim=-1时， 是对某一维度的行进行softmax运算：
        self.init_weights()
...
class CAM_Calculate(nn.Module):
    ...
    def forward(self, x):
        ...
        energy = torch.bmm(proj_query, proj_key)
        energy_new = torch.max(energy, -1, keepdim=True)[0].expand_as(energy) - energy#-1）代表按最细粒度的方向
...
class CAM_Use(nn.Module):
    ...
    def __init__(self, in_dim):
        ...
        self.chanel_in = in_dim
        self.gamma = nn.Parameter(torch.zeros(1))#  gamma参数初始化为0,可学习
